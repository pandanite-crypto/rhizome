 ...
 * @see RpcServer
 */
public final class PeerClient implements IRpcClient, EventloopService, WithInitializer<PeerClient>, EventloopJmxBeanWithStats {
	private static final boolean CHECK = Checks.isEnabled(PeerClient.class);

	public static final SocketSettings DEFAULT_SOCKET_SETTINGS = SocketSettings.createDefault();
	public static final Duration DEFAULT_CONNECT_TIMEOUT = ApplicationSettings.getDuration(PeerClient.class, "connectTimeout", Duration.ZERO);
	public static final Duration DEFAULT_RECONNECT_INTERVAL = ApplicationSettings.getDuration(PeerClient.class, "reconnectInterval", Duration.ZERO);
	public static final MemSize DEFAULT_PACKET_SIZE = ApplicationSettings.getMemSize(PeerClient.class, "packetSize", ChannelSerializer.DEFAULT_INITIAL_BUFFER_SIZE);
	...
	private final Map<InetSocketAddress, PeerClientConnection> connections = new HashMap<>();
	...
	// region builders
	private PeerClient(Eventloop eventloop) {
		this.eventloop = eventloop;
	}

	public static PeerClient create(Eventloop eventloop) {
		return new PeerClient(eventloop);
	}

	public PeerClient withClassLoader(ClassLoader classLoader) {
	 ...
	 */
	public PeerClient withSocketSettings(SocketSettings socketSettings) {
	 ...
	 */
	public PeerClient withMessageTypes(Class<?>... messageTypes) {
	 ...
	 * message types
	 */
	public PeerClient withMessageTypes(List<Class<?>> messageTypes) {
	 ...
	 */
	public PeerClient withSerializerBuilder(SerializerBuilder serializerBuilder) {
	 ...
	 */
	public PeerClient withStrategy(RpcStrategy requestSendingStrategy) {
		this.strategy = requestSendingStrategy;
		return this;
	}

	public PeerClient withStreamProtocol(MemSize defaultPacketSize) {
		this.defaultPacketSize = defaultPacketSize;
		return this;
	}

	public PeerClient withStreamProtocol(MemSize defaultPacketSize, @Nullable FrameFormat frameFormat) {
		this.defaultPacketSize = defaultPacketSize;
		this.frameFormat = frameFormat;
		return this;
	}

	public PeerClient withAutoFlush(Duration autoFlushInterval) {
		this.autoFlushInterval = autoFlushInterval;
		return this;
	}

	public PeerClient withKeepAlive(Duration keepAliveInterval) {
	 ...
	 */
	public PeerClient withConnectTimeout(Duration connectTimeout) {
		this.connectTimeoutMillis = connectTimeout.toMillis();
		return this;
	}

	public PeerClient withReconnectInterval(Duration reconnectInterval) {
		this.reconnectIntervalMillis = reconnectInterval.toMillis();
		return this;
	}

	public PeerClient withSslEnabled(SSLContext sslContext, Executor sslExecutor) {
		this.sslContext = sslContext;
		this.sslExecutor = sslExecutor;
		return this;
	}

	public PeerClient withLogger(Logger logger) {
		this.logger = logger;
		return this;
	}
	 ...
	 * availability
	 */
	public PeerClient withForcedStart() {
	 ...
	 */
	public PeerClient withForcedShutdown() {
	...
	private void doStop() {
		...
		for (PeerClientConnection connection : connections.values()) {
			if (forcedShutdown) {
	...
	private Promise<Void> connect(InetSocketAddress address) {
		return AsyncTcpSocketNio.connect(address, connectTimeoutMillis, socketSettings)
				.whenResult(asyncTcpSocketImpl -> {
					...
					PeerClientConnection connection = new PeerClientConnection(eventloop, this, address, stream, keepAliveInterval.toMillis());
	...
	public void startMonitoring() {
		monitoring = true;
		for (InetSocketAddress address : addresses) {
			PeerClientConnection connection = connections.get(address);
			if (connection != null) {
				connection.startMonitoring();
			}
		}
	...
	public Map<InetSocketAddress, PeerClientConnection> getRequestStatsPerConnection() {
		return connections;
	}
