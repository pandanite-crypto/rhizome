...
@Getter
@Setter
public class PeerServer extends AbstractServer<PeerServer> {
	...
	private final List<PeerServerConnection> connections = new ArrayList<>();
    ...
    // CONSTRUCTOR REGION
    private PeerServer(Eventloop eventloop) {
		super(eventloop);
	}
    
	public static PeerServer create(Eventloop eventloop) {
		return new PeerServer(eventloop)
                    .withServerSocketSettings(DEFAULT_SERVER_SOCKET_SETTINGS)
                    .withSocketSettings(DEFAULT_SOCKET_SETTINGS);
	}

	public PeerServer withClassLoader(ClassLoader classLoader) {
	 ...
	 */
	public PeerServer withMessageTypes(Class<?>... messageTypes) {
	 ...
	 * @return server instance capable for handling provided message types
	 */
	public PeerServer withMessageTypes(@NotNull List<Class<?>> messageTypes) {
		checkArgument(new HashSet<>(messageTypes).size() == messageTypes.size(), "Message types must be unique");
		this.messageTypes = messageTypes;
		return this;
	}

	public PeerServer withSerializerBuilder(SerializerBuilder serializerBuilder) {
	 ...
	 */
	public <I, O> PeerServer withHandler(Class<I> requestClass, RpcRequestHandler<I, O> handler) {
	...
	public PeerServer withHandlers(@NotNull Map<Class<?>, RpcRequestHandler<?, ?>> handlers) {
	...
	protected void serve(AsyncTcpSocket socket, InetAddress remoteAddress) {
		...
		var connection = new PeerServerConnection(this, remoteAddress, handlers, stream);
	...
	protected void onClose(SettablePromise<Void> cb) {
		if (connections.isEmpty()) {
			...
			for (PeerServerConnection connection : new ArrayList<>(connections)) {
				connection.shutdown();
			}
			closeCallback = cb;
		}
	...
	void add(PeerServerConnection connection) {
	...
	boolean remove(PeerServerConnection connection) {
	...
	public void startMonitoring() {
		monitoring = true;
		for (PeerServerConnection connection : connections) {
			connection.startMonitoring();
		}
	}
	...
	public List<PeerServerConnection> getConnections() {
		return connections;
	}
